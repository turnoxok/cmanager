...
document.getElementById('videoInput').addEventListener('change', e => {
  ...
  video.loop = true; // solo para previsualización
  video.muted = false;
  video.play();
...
document.getElementById('exportBtn').addEventListener('click', () => {
  ...
  // Preparar grabación
  video.loop = false;       // no repetir
  video.currentTime = 0;    // desde el principio
  video.play();             // asegurar que avance
  ...
  mediaRecorder.start();
  alert("Grabando video completo... Presiona OK y espera que termine.");

  const duration = video.duration*1000; // duración exacta
  setTimeout(()=>mediaRecorder.stop(), duration + 200); // extra 200ms para último frame
});
