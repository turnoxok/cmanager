...
document.getElementById("videoInput").addEventListener("change",e=>{
  ...
  video.addEventListener("loadedmetadata",()=>{
    ...
    if(videoRatio>WIDTH/HEIGHT){videoH=HEIGHT;videoW=videoH*videoRatio;videoX=(WIDTH-videoW)/2;videoY=0;}
    else{videoW=WIDTH;videoH=videoW/videoRatio;videoX=0;videoY=(HEIGHT-videoH)/2;}
  });
});
...
document.getElementById("logoInput").addEventListener("change",e=>{
  ...
  reader.onload=ev=>{
    logo=new Image();
    logo.onload=()=>{
      logoW=250; logoH=logoW*(logo.height/logo.width);
...
function getPos(e){
  ...
  if(e.touches)return [(e.touches[0].clientX-rect.left)*(WIDTH/rect.width),(e.touches[0].clientY-rect.top)*(HEIGHT/rect.height)];
...
function moveDrag(e){
  ...
  if(e.touches && e.touches.length===2 && lastDist){
    ...
    else{const cx=videoX+videoW/2,cy=videoY+videoH/2;videoW*=zoom;videoH=videoW/videoRatio;videoX=cx-videoW/2;videoY=cy-videoH/2;}
    lastDist=dist;
  }else{
...
document.getElementById("exportBtn").addEventListener("click",async()=>{
  ...
  try{
    ...
    alert("Error al exportar el video: "+err.message);
  }
});
